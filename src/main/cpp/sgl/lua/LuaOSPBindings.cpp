...
LuaOSPBindings::LuaOSPBindings(const std::string& scriptPath) :
...
void LuaOSPBindings::Load()
{
    ...
    KS_LOG_DEBUG("Loaded output state processor script: %s", m_scriptPath);
}

void LuaOSPBindings::Unload()
{
    m_device.clear();
    m_screen.clear();
...
void LuaOSPBindings::LDeactivate()
{
...
void LuaOSPBindings::LProcess(const io::OutputState& src, io::OutputState& dest)
{
    lua_rawgeti(m_state->GetState(), LUA_REGISTRYINDEX, m_funcProcess);

    LuaIOPHelper::AddOutputStateToStack(m_state, src);

    if (lua_pcall(m_state->GetState(), 1, 1, 0)) {
        __ThrowCallException("Process");
    }

    LuaIOPHelper::GetOutputStateFromStack(m_state, dest);
}

void LuaOSPBindings::__ThrowCallException(const std::string& function)
{
