...
local function __load(table_resources, table_args)
    ...
    if res_bgm_title == nil then
        print("Missing bgm_title resource entry")
        return false
    end
    ...
    res_bgm_title = asset_paths["aud"] .. "/" .. res_bgm_title
    ...
    -- load bgm
    __id_bgm_title = sgl_snd_create(res_bgm_title)
    sgl_snd_loop(__id_bgm_title, -1)
...
local function __unload()
    ...
    sgl_snd_destroy(__id_bgm_title)
end
